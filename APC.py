    ...
    for s in range(len(RIRs)):
        ...
        for c in range(channels[s]): 
            ...
            for b in range(len(bands)+1):
                ...
                IRstart = np.where(signalsdb[s][c][b][0:np.argmax(signalsdb[s][c][b])]<=np.max(signalsdb[s][c][b])/10)[0]
                if len(IRstart)==0:
                    IRstart=0
                else:
                    IRstart=IRstart[-1]
                ...
                if c==1:
                    iacc = np.correlate(signals[s][0][b],signals[s][1][b]) / np.sqrt(np.sum(signals[s][0][b]**2)*np.sum(signals[s][0][b]**2))
...
def func_exptable():
    if len(cmbx_IRplt["values"])!=0:
        filename = filedialog.asksaveasfilename(title="Save current table", filetypes=[("CSV File", "*.csv")],defaultextension=[("CSV File", "*.csv")])
...
def func_expplot():
    if len(cmbx_IRplt["values"])!=0:
        filename = filedialog.asksaveasfile(title="Save current plot",filetypes=[("PNG Image","*.png")],defaultextension=[("PNG Image","*.png")])
        if not filename: return
        if btn_show["text"]=="Show average":
